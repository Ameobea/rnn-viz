...
use crate::batch_activation::{apply_ameo, apply_soft_leaky_ameo};
...
fn activation_from_id(id: usize) -> fn(f32) -> f32 {
  match id {
    ...
    5 => apply_ameo,
    // soft leaky ameo
    6 => |x: f32| apply_soft_leaky_ameo(0.05, x),
  ...
  for x_ix in 0..size {
    ...
    for y_ix in 0..size {
      ...
      let color = match activated {
        // decision boundary
        x if (decision_boundary - x).abs() < 0.005 => [0xff, 0xff, 0xff, 0xff],
        // perfect positive
        x if (positive_target_value - x).abs() < 0.005 => [0, 0xcc, 0, 0xff],
        // perfect negative
        x if (negative_target_value - x).abs() < 0.005 => [0xff, 0, 0, 0xff],
        // negative
        x if x < decision_boundary => {
          let diff = (decision_boundary - x).abs().min(decision_range / 2.) * 1.;
          [
